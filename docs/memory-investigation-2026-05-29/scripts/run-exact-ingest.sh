...
VERIFY_ID="${VERIFY_ID:-V00-exact-ingest}"
...
APP_PORT="${APP_PORT:-6160}"
DB_PORT="${DB_PORT:-3466}"
...
sleep 30
sample_once "$APP_CONTAINER" "$DB_CONTAINER" "post-import-idle-30s"
collect_container_evidence "$APP_CONTAINER" "$DB_CONTAINER"
...
- Follow-up: run endpoint probes against http://127.0.0.1:${APP_PORT} before cleanup if this dataset is needed
EOF

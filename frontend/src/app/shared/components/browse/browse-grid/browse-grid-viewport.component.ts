...
export class BrowseGridViewportComponent implements OnInit {
  ...
  readonly skeletonFill = input(false);
    ...
    runInInjectionContext(this.injector, () => {
      this.grid.set(
        createVirtualGrid({
          ...
          count: computed(() => Math.max(this.count() ?? 0, this.items().length)),
  ...
  private enterWindowMode(): void {
    ...
    this.scrollElementRef.set(new ElementRef(scrollElement as HTMLElement));
    ...
    if (typeof ResizeObserver !== 'undefined') {
      const target = this.hostRef.nativeElement.closest<HTMLElement>('.app-page') ?? document.body;
      marginObserver = new ResizeObserver(measureMargin);

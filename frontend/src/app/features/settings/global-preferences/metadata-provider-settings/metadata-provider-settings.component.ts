...
const PROVIDERS_WITH_OPTIONS: ReadonlySet<MetadataProviderId> = new Set([
  'Amazon', 'Google', 'Hardcover', 'Comicvine', 'Ranobedb', 'Audible', 'AppleBooks',
]);
...
export class MetadataProviderSettingsComponent {
  ...
  readonly configurableProviders = computed(() => this.catalogue.providers().filter(provider => PROVIDERS_WITH_OPTIONS.has(provider.id)));
  readonly simpleProviders = computed(() => this.catalogue.providers().filter(provider => !PROVIDERS_WITH_OPTIONS.has(provider.id)));
    ...
    const payload = [
      {
        ...
        newValue: Object.fromEntries(this.catalogue.providers().map(provider => [

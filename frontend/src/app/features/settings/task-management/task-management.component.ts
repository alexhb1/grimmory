...
export class TaskManagementComponent implements OnInit {
  ...
  private sortTasksByDisplayOrder(tasks: TaskInfo[]): TaskInfo[] {
    return tasks.sort((a, b) => {
      const orderA = TASK_TYPE_CONFIG[a.taskType]?.displayOrder ?? 999;
      const orderB = TASK_TYPE_CONFIG[b.taskType]?.displayOrder ?? 999;
  ...
  private runTaskWithOptions(type: TaskType, options: LibraryRescanOptions | MetadataRefreshRequest | null): void {
    ...
    const isAsync = TASK_TYPE_CONFIG[type]?.async || false;
  ...
  getTaskDisplayOrder(type: TaskType): number {
    return TASK_TYPE_CONFIG[type]?.displayOrder ?? 999;
  }

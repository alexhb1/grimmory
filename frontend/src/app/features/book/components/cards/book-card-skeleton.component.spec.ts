...
describe('BookCardSkeletonComponent', () => {
  ...
  it('renders a title and an author row by default', () => {
    expect(rowHeights()).toEqual([BOOK_CARD_META_TITLE_HEIGHT, BOOK_CARD_META_AUTHOR_HEIGHT]);
  });

  it('adds an accessory row for a third meta line', () => {
    setMetaLines(3);
  ...
  it('follows the card aspect for the cover block', () => {
    const cover = (): string =>
      (host.querySelector('[data-testid="skeleton-cover"]') as HTMLElement).className;

    expect(cover()).toContain('aspect-[5/7]');

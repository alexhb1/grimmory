...
describe('BookBrowsePageComponent', () => {
  ...
  function bulkHost(): BulkHarness {
    return fixture.debugElement.query(By.directive(BookBrowseBulkBarComponent))
      .componentInstance as unknown as BulkHarness;
  }
  ...
  function headerActionLabels(root: ParentNode): string[] {
    ...
    return Array.from(menu?.querySelectorAll('app-menu-item') ?? [])
      ...
      .filter(label => label !== 'Columns' && label !== 'Card Detail');
  }
  ...
  function moreTrigger(): HTMLButtonElement | null {
    return host().querySelector('button[aria-label="More Actions"]');
  }

  function fitAllBulkVerbs(): void {
    bulkHost().bulkBar()?.availableWidth.set(2_000);
    fixture.detectChanges();
  }
  ...
  it('pins a library route scope as a facet the user cannot widen or remove', async () => {
    currentUser.set(userFixture({canManageLibrary: true}));
    fixture.destroy();
  ...
  it('keeps the previous results visible until the incoming covers are ready', async () => {
    fixture.detectChanges();
    expectInitialPageRequest().flush(summaryPage([book(1), book(2)]));
    await flushQueryAsync();
    fixture.detectChanges();

    await TestBed.inject(Router).navigate([], {queryParams: {facet: 'genre:Fantasy'}});
    fixture.detectChanges();
    flushFacetRegistry();

    expect(page().books().length).toBe(2);
    expect(page().status()).toBe('success');
    expect(page().booksQuery.isPlaceholderData()).toBe(true);

    http.expectOne(request =>
      request.url === PAGE_URL && request.params.get('facet') === 'genre:Fantasy',
    ).flush(summaryPage([book(3)]));
    fixture.detectChanges();
    ...
    await vi.waitFor(() => {
      fixture.detectChanges();
      expect(page().books().length).toBe(1);
    });
    expect(page().booksQuery.isPlaceholderData()).toBe(false);
  });

  it('drops to the skeleton when a collection change outlasts the hold patience', async () => {
    fixture.detectChanges();
    expectInitialPageRequest().flush(summaryPage([book(1), book(2)]));
    await flushQueryAsync();
    fixture.detectChanges();
    ...
    await vi.waitFor(() => {
      fixture.detectChanges();
      expect(page().books().length).toBe(0);
  ...
  it('normalizes punctuation in page requests while preserving the raw query URL param', async () => {
    await TestBed.inject(Router).navigate([], {queryParams: {query: 'dune!'}});
    fixture.detectChanges();
    flushFacetRegistry();

    http.expectOne(candidate =>
      candidate.url === PAGE_URL && candidate.params.get('query') === 'dune',
    ).flush(bookPage([1], 1));
    await flushQueryAsync();
  ...
  it('shows the active query as a removable search chip', async () => {
    ...
    const chipEl = searchChip.nativeElement as HTMLElement;
    expect(chipEl.textContent).toContain('Search:');
    expect(chipEl.textContent).toContain('the warden');

    (searchChip.query(By.css('button')).nativeElement as HTMLButtonElement).click();
  ...
  it('shows a removable raw-label chip for a facet outside the loaded vocabulary', async () => {
    ...
    const chipEl = chip.nativeElement as HTMLElement;
    expect(chipEl.textContent).toContain('Read status:');
    expect(chipEl.textContent).toContain('READ');

    (chip.query(By.css('button')).nativeElement as HTMLButtonElement).click();
    expect(navigate).toHaveBeenCalledWith([], expect.objectContaining({
  ...
  it('clears the query immediately without a late debounced navigation', () => {
    vi.useFakeTimers();
    try {
      fixture.detectChanges();
      expectInitialPageRequest().flush(bookPage([1], 1));
  ...
  it('shows only lock actions in Metadata for a lock-only user', async () => {
    currentUser.set(userFixture({canBulkLockUnlockMetadata: true}));
  ...
  it('shows Organize Files only with its permission and local disk storage', async () => {
    ...
    currentUser.set(userFixture({canMoveOrganizeFiles: true}));
  ...
  it('clears Attach Files when a query change clears the selection', async () => {
    currentUser.set(userFixture({admin: true}));

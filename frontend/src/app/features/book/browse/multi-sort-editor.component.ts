...
export class MultiSortEditorComponent {
  ...
  readonly terms = model<readonly BookSortTerm[]>([]);
  readonly options = input<readonly BookSortOption[]>([]);
    ...
    return this.terms().map(term => {
      ...
      return {
        ...
        icon: sortDirectionIcon({
          option: option ?? {
            id: term.key,
            fallbackLabel: term.key,
            group: 'more',
            defaultDirection: term.direction,
            directions: [term.direction],
          },
          direction: term.direction,
        }),
        canFlip: option ? option.directions.length > 1 : true,
      };
    });
  });
  ...
  private labelFor(option: BookSortOption | undefined, key: string): string {
    ...
    return option.labelKey ? this.transloco.translate(option.labelKey) : option.fallbackLabel;
  }

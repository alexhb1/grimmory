...
export class BookBrowseColumnPreferenceService {
  ...
  get allColumns(): BookBrowseColumn[] {
    return FIELDS.map(field => ({field, header: this.transloco.translate(`browse.table.columns.${field}`)}));
  }
  ...
  get visibleColumns(): BookBrowseColumn[] {
    return this.preferences()
      ...
      .map(preference => ({
        ...
        header: this.transloco.translate(`browse.table.columns.${preference.field}`),
      }));
  }

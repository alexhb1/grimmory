...
describe('bookSortLineAvailable', () => {
  it('skips the keys the card already shows at rest', () => {
    expect(bookSortLineAvailable('title')).toBe(false);
    expect(bookSortLineAvailable('seriesNumber')).toBe(false);
  });

  it('offers a line for configured formatter keys', () => {
    expect(bookSortLineAvailable('addedOn')).toBe(true);
    expect(bookSortLineAvailable('readStatus')).toBe(true);
    expect(bookSortLineAvailable('goodreadsRating')).toBe(true);
  });

  it('does not add a line for unknown sort keys', () => {
    expect(bookSortLineAvailable('futureScore')).toBe(false);
  });
});
...
describe('BookBrowseSortLineService', () => {
  ...
  it('renders every absent value as the muted dash', () => {
    ...
    expect(service.lineFor('futureScore', bare)).toBe('—');
  });
});

...
describe('AuthorService', () => {
  ...
  it('refreshes embedded book author data after an author rename', () => {
    ...
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({queryKey: ['books', 'detail']});
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({queryKey: ['books', 'recommendations']});
  ...
  it('refreshes embedded book author data after authors are deleted', () => {
    service.deleteAuthors([7, 8]).subscribe();

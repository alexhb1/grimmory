...
class BookFacetServiceTest {
    ...
    void linksCarryToggleHrefAndCount() {
        ...
        FacetLink horror = group(facets(null, null, null), "genre").links()
    ...
    void facetValueLinksPreserveMustAndNotSelections() {
        ...
        FacetLink history = group(
                facets(null, List.of("genre:History"), List.of("genre:Romance"), null, null),
                "genre").links().stream().filter(link -> "History".equals(link.value())).findFirst().orElseThrow();

        assertThat(history.href()).isEqualTo(
                "/api/v1/books/page?facet_must=genre%3AHistory&facet_not=genre%3ARomance&facet=genre%3AHistory");
    }
